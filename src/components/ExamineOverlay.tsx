...
function ChatTab({ patientName }: { patientName: string }) {
  ...
  useEffect(() => {
    ...
    setIsVoiceDisabled(conv.isVoiceDisabled?.() ?? false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      await conv.sendTextMessage(text);
    } catch (err) {
      console.error(err);
    } finally {
      setSending(false);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {visible.length === 0 ? (
        <div className="plush" style={{ padding: 14, fontWeight: 700, color: 'var(--ink-2)', minHeight: 60 }}>
      ...
      >
        <input
          ...
          placeholder={sending ? 'Thinking...' : isVoiceDisabled ? 'Type a message...' : 'Type a message (or speak)...'}
          disabled={sending}
          style={{
            flex: 1,
            padding: '10px 12px',
