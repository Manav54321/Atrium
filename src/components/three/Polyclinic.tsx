...
const DESK_POS: [number, number, number] = [0.8, 0, ROOM_BACK_Z + 2.0];
const DESK_ROT_Y = 0.35;
/** Doctor sits behind the desk (closer to the back wall). */
export const DOCTOR_CHAIR_POS: [number, number, number] = [1.2, 0, ROOM_BACK_Z + 1.2];
export const PATIENT_CHAIR_POS: [number, number, number] = [-1.0, 0, ROOM_BACK_Z + 3.8];
...
function Floor() {
  // A futuristic metal floor grid with glowing neon blue/cyan conduits.
  const GRID_SIZE = 1.0;
  const xLinesCount = Math.ceil(OUTER_WIDTH / GRID_SIZE) + 1;
  const zLinesCount = Math.ceil(OUTER_DEPTH / GRID_SIZE) + 1;
  
  const lines: JSX.Element[] = [];
  
  // Render grid lines running along Z
  for (let i = 0; i < xLinesCount; i++) {
    const x = WORLD_LEFT_X + i * GRID_SIZE;
    lines.push(
      <mesh
        key={`x-line-${i}`}
  ...
  // Render grid lines running along X
  for (let i = 0; i < zLinesCount; i++) {
    const z = WORLD_BACK_Z + i * GRID_SIZE;
    lines.push(
      <mesh
        key={`z-line-${i}`}
        rotation={[-Math.PI / 2, 0, 0]}
  ...
  return (
    <>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow position={[OUTER_CENTER_X, 0, OUTER_CENTER_Z]}>
        ...
        <meshStandardMaterial color={PALETTE.floor} roughness={0.9} metalness={0.8} />
      </mesh>
      {lines}
    </>
  );
...
/** Canvas texture for the consultation desk monitor. Renders a futuristic medical
 *  interface with real-time patient telemetry waveforms, diagnostic charts,
 *  and session constraints under the dark cyber clinical palette. */
function makeMonitorTexture(patient: MonitorPatient | null): CanvasTexture {
  ...
  if (comp.length > 28) {
    ctx.fillText(comp.substring(0, 26) + '...', cardX + 12, cardY + 116);
  } else {
    ctx.fillText(comp, cardX + 12, cardY + 116);
  }
  ...
  ctx.beginPath();
  let wx = telX + 8;
  ctx.moveTo(wx, telY + telH / 2);
  for (let i = 0; i < 40; i++) {
    const step = telW / 40;
    const px = wx + i * step;
    let py = telY + telH / 2;
    if (i % 8 === 2) {
      py -= 15; // ECG QRS complex spike
    } else if (i % 8 === 3) {
      py += 22;
    } else if (i % 8 === 4) {
      py -= 4;
...
function makeWindowViewTexture(): CanvasTexture {
  ...
  sky.addColorStop(0.5, '#070b19');
  sky.addColorStop(1, '#0e091a');
  ctx.fillStyle = sky;
  ctx.fillRect(0, 0, w, h);
  ...
  while (bx < w) {
    ...
    ctx.fillStyle = Math.random() > 0.5 ? '#00f5d4' : '#ff0054';
    ctx.fillRect(bx + bw / 2 - 1, h - bh + 10, 2, bh - 20);
    
    // window arrays
    ctx.fillStyle = 'rgba(0, 245, 212, 0.3)';
    for (let wy = h - bh + 15; wy < h - 15; wy += 20) {
  ...
  useEffect(() => {
    if (patientName) {
      interactionBus.register({
        ...
        radius: 100,
        prompt: `E — Examine   ·   S — Talk to ${patientName}`,
        kind: 'bed',
  ...
  useFrame((_, dt) => {
    ...
    if (phase === 'walk-in') {
      ...
      if (t >= 1) {
        ...
        g.rotation.y = DESK_ROT_Y + Math.PI - 0.2;
      }
...
export function Polyclinic({
  ...
  return (
    <>
      ...
      />
      <DoctorChair position={DOCTOR_CHAIR_POS} rotationY={DESK_ROT_Y} />
      ...
      <PatientChair position={PATIENT_CHAIR_POS} rotationY={DESK_ROT_Y + Math.PI - 0.2} />
